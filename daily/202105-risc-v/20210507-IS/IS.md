...
对于一个过程，可能需要使用的寄存器超过六个参数寄存器和两个返回值寄存器，这时候就需要使用其他的通用寄存器，但是在过程调用后caller仍然需要这些寄存器的被使用前的值，这时候就需要让callee使用的任何寄存器都恢复到过程调用前所储存的值。

而能换出寄存器值的最理想的数据结构就是栈——先入后出的队列结构。

栈需要一个指针指向最新分配的地址，在每次寄存器进行保存和恢复时，栈指针都需要以当前设备指令集体系的位数的倍数进行调整。RISC-V中使用寄存器x2作为栈指针，即$sp；压栈和出栈。

$t 寄存器是临时寄存器，调用callee前后不需要保存和回复；

$s 寄存器是保存寄存器，调用callee时默认这些寄存器里面的值是caller需要的，就需要保存和恢复；



##### 8.2 嵌套过程（非叶过程）

嵌套过程中会出现参数寄存器和返回地址寄存器使用过程出现冲突的情况；解决这个问题的办法就是将调用前将其他所有必须保留的寄存器压栈。
