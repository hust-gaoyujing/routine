...
![image-20201229152254664](uart_spec.assets/uart整体框图.png)



- 门控时钟--uart_clk

  ​	这个模块用于为UART的提供门控时钟，其中门控使能收到uart_ctrl寄存器中的bit9控制，当门控使能拉低时，uart模块除uart_bus_codec模块外其余模块均无时钟输入。
...
- 发送器--uart_tx
  ...
  ​	在模块使能打开的情况下，接收器时刻监测是否有数据通过总线写入数据寄存器，如果有则将数据寄存器的数据load到移位寄存器中，并开始按照配置的帧格式开始发送数据，数据从TXD线输出，当这一帧数据发送完成，模块将反馈一个完成状态tx_ok回uart_bus_codec模块的uart_csr寄存器，此时可以通过总线读取到此完成状态并开始下一byte的数据写入和发送。

- 接收器--uart_rx

  ​	在模块使能打开的情况下，接收器时刻检测RXD线的电平，当检测到起始位，接收器启动接收状态机和奇偶校验状态机，当数据收集和校验成功下，接收器开始检测RXD线的停止位，当检测到停止位，接收器将反馈一个完成状态rx_ok回uart_bus_codec模块的uart_csr寄存器，此时可以通过总线读取到此完成状态。其中接收器的采样频率是发送器接收频率的16倍频，表示完成一个bit数据接收需要16个baudrate时钟，收集其中三个时刻的电平并作多数表决作为此bit的电平值。
...
​		门控时钟电路图如下，其中uart_clk_en使能信号受到uart_bus_ctrl中uart_ctrl寄存器的bit9控制。

​		                                                       <img src="uart_spec.assets/uart_clk.jpg" alt="uart_clk"  />



##### 5.2  控制模块--uart_bus_codec
...
- baud_en为该模块的使能信号，由uart_bus_codes模块提供；

- divisor为uart_rx信号的分频计数值，即通过这个分频值所得到的时钟频率是波特率的16倍，由uart_bus_codes模块提供；
...
​		发送器采用系统门控时钟作为时序电路的采样时钟，当检测到tx_start信号（即uart_bus_codec中的wr_data_flag信号)，内部标志信号tx_on_flag置位；此时开始等待波特率时钟到来，当tx_data_sample上升沿到来时，状态机跳转至数据发送状态，当数据发送完成根据配置选择进入校验位发送或者直接进入停止位发送，如果需要发送校验位，则等待检验位发送完成继续进入停止位发送。当停止位发送完成，向uart_bus_codec模块反馈tx_ok信号，进入IDLE状态并等待下一个帧数据的到来。
...
- tx_data_sample为波特率时钟上升沿信号，由uart_br_gen提供；

- tx_en为该模块的使能信号，由uart_bus_codes模块提供；
- no_parity，ev_parity信号是奇偶校验的相关信号，由uart_bus_codes模块中uart_ctrl寄存器提供；
- tx_start为每一帧数据发送的启动信号，当icb总线向data_reg写数据时，该信号置位；
- txd_in信号为数据输入端口，当满足状态机从IDLE向START跳转条件时，data_reg寄存器的数据通过该端口load进入移位寄存器；
- tx_ok表示一帧数据发送完成的反馈信号，反馈回uart_bus_codes模块中的uart_csr寄存器，可通过icb总线读取；
- txd为uart_tx的数据发送端口；
...
​		发送器采用系统门控时钟作为时序电路的采样时钟，等到rx_data_sample信号的上升沿到来后采样一次RXD线，当RXD线出现一次下降沿，即接收到起始位，则开始接收数据。byte数据采样依旧使用多数表决电路进行采样。数据接收完成后，如果有配置有校验位，则开始接收校验位并进行校验位的判断，如失败则返回IDLE状态继续等待下一个起始位的到来且返回一个parity_error；如果没有配置校验位，则直接进入停止位接收，当停止位接收完成后，返回IDLE状态，并且反馈tx_ok给uart_bus_codec中的uart_csr寄存器，此时可以通过总线读取data_reg获取接收的数据；
...
- rx_data_sample为接收器采样时钟上升沿信号，由uart_br_gen提供；

- rx_en为该模块的使能信号，由uart_bus_codes模块提供；
...
- rx_ok表示一帧数据接收完成的反馈信号，反馈回uart_bus_codes模块中的uart_csr寄存器，可通过icb总线读取；
...
shift_data_reg <= {shift_data_reg[7:1],tx};
...
- bit16-31    divisor              R/W     标志此时uart_clk需要分频多少才能得到rx采样时钟，得到采样            

  ​                                                         时钟，得到采样时钟后再进行十六分频即可得到波特率时钟；
...
- bit4			tx_en			 R/W		  为高时标志tx模块使能打开，为低时标志tx模块使能关闭；
...
- bit0-7		data_reg			R/W	当此寄存器写入新数据，wr_data_flag置位，标志uart有一                  

  ​															组新数据需要发送，当tx_ok拉高时，表示此时这一帧数据已  															经发送完成，可以开始下一帧数据的发送；当总线读到															uart_csr寄存器中的rk_ok状态位置位时表示一帧数据接收完															成，此时读data_reg寄存器即可获取接受的数据； 
