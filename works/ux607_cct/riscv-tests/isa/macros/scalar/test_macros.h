...
test_ ## testnum: \
      li  x1, MASK_XLEN(val1); \
      li  x23, MASK_XLEN(val2); \
      li  t3, MASK_XLEN(result1); \
      li  t4, MASK_XLEN(result2); \
      inst1 t5, x1, x23; \
      inst2 t6, x1, x23; \
      li  TESTNUM, testnum;  \
      bne t3, t5, fail;   \
...
#define TEST_STREAM_LD_ST_OP( testnum, load_inst, store_inst, result, base ) \
    TEST_CASE( testnum, x30, result, \
      ...
      li  x23, result; \
      store_inst x23,    0(x1); \
      store_inst x23,   16(x1); \
      store_inst x23,   32(x1); \
      store_inst x23,   48(x1); \
      store_inst x23,   64(x1); \
      store_inst x23,   80(x1); \
      store_inst x23,   96(x1); \
      store_inst x23,  112(x1); \
      store_inst x23,  128(x1); \
      store_inst x23,    0(x1); \
      store_inst x23,   16(x1); \
      store_inst x23,   32(x1); \
      store_inst x23,   48(x1); \
      ...
      load_inst x30,    80(x1); \
      load_inst x30,    96(x1); \
      load_inst x30,   112(x1); \
      load_inst x30,   128(x1); \
      load_inst x30,     0(x1); \
      load_inst x30,    16(x1); \
      load_inst x30,    32(x1); \
      load_inst x30,    48(x1); \
...
1:  li  x1, result; \
    ...
    la  x23, base; \
    TEST_INSERT_NOPS_ ## src2_nops \
    store_inst x1, offset(x23); \
    load_inst x30, offset(x23); \
    li  x29, result; \
    bne x30, x29, fail; \
    addi  x4, x4, 1; \
    li  x5, 2; \
