...
static void write_tohost()
{
  while(1)
  {
     write_csr(sscratch, 1);
     set_csr(sstatus, MSTATUS_SIE);
     asm volatile ("sw tp, (%0)" :: "r"(&tohost) : "memory");
  }
}
...
void handle_fault(uintptr_t addr, uintptr_t cause)
{
  ...
  // first flush_page is to create page for kernel
  ...
  // second flush_page is to create page for user
...
# error
  ...
  for(long i = 0; i < MAX_TEST_PAGES; i++){
    user_llpt[i] = 0;
    user_mapping[i].addr = 0;
    user_mapping[i].next = 0;
  }
#elif SATP_MODE_CHOICE == SATP_MODE_SV32
