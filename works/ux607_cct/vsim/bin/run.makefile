...
SIM_OPTIONS   := +v2k -sverilog -notice -q +lint=all,noSVA-NSVU,noPCTIO-L,noVCDE,noUI,noSVA-CE,noSVA-DIU,noPORTFRC,noSVA-ICP,noNS +define+ENABLE_TB_FORCE ${COV_OPT} -debug_access+all -full64 -timescale=1ns/10ps ${CODE_COV}
SIM_OPTIONS   += +incdir+${VSRC_DIR}/core 
SIM_OPTIONS   += -l compile.log
SIM_OPTIONS   += +incdir+"${VSRC_DIR}/core/"+"${VSRC_DIR}/core/GNRL/"+"${VSRC_DIR}/subsys/"+"${VSRC_DIR}/perips/"+"${VSRC_DIR}/soc/"+"${VSRC_DIR}/fab/"+"${VSRC_DIR}/mems"+"${COVGRP_DIR}"+"${VTB_DIR}"
ifeq ($(SMIC130LL),1) 
...
wave: 
	...
	gvim  ${TESTCASE}.dump &
ifeq ($(COVERAGE), 1)
	${WAV_TOOL} ${WAV_OPTIONS} +incdir+"${VSRC_DIR}/core/"+"${VSRC_DIR}/subsys/"+"${VSRC_DIR}/perips/"+${COVGRP_DIR}+"${VTB_DIR}"  ${RTL_V_FILES} ${TB_V_FILES} ${COVGRP_SV_FILES} -ssf ${TEST_RUNDIR}/tb_top.${WAV_PFIX} -top tb_top -nologo & 
else
	${WAV_TOOL} ${WAV_OPTIONS} +incdir+"${VSRC_DIR}/core/"+"${VSRC_DIR}/core/GNRL/"+"${VSRC_DIR}/subsys/"+"${VSRC_DIR}/perips/"+${COVGRP_DIR}+"${VTB_DIR}"  ${RTL_V_FILES} ${TB_V_FILES} -ssf ${TEST_RUNDIR}/tb_top.${WAV_PFIX} -top tb_top -nologo & 
endif
