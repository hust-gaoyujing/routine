...
object RegMap {
  ...
  def generate(mapping: Map[Int, (UInt, UInt => UInt)], raddr: UInt, ren: Bool, rdata: UInt,
    waddr: UInt, wen: Bool, wdata: UInt, wmask: UInt):Unit = {
    val chiselMapping = mapping.map { case (a, (r, w)) => (a.U, r, w) }
    when(ren) { rdata := LookupTree(raddr, chiselMapping.map { case (a, r, w) => (a, r) }) }
    chiselMapping.map { case (a, r, w) =>
      if (w != null) when (wen && waddr === a) { r := w(MaskData(r, wdata, wmask)) }
    }
  }
  def generate(mapping: Map[Int, (UInt, UInt => UInt)], addr: UInt, ren: Bool, rdata: UInt,
    wen: Bool, wdata: UInt, wmask: UInt):Unit = generate(mapping, addr, ren, rdata, addr, wen, wdata, wmask)
}
